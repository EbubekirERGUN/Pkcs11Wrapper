...
| Crypto API host scaffold | ✅ | stateless ASP.NET Core host with DI/config, service documents, health/readiness, PostgreSQL-backed shared auth/policy persistence, optional Redis hot-path acceleration, hashed API-key lifecycle controls, and the first alias-routing/policy-enforcement slice |
