...
- `eng/run-regression-tests.sh --use-existing-env` skips fixture provisioning and uses existing `PKCS11_*` environment variables. This is intended for optional vendor-module validation and defaults to the `baseline-rsa-aes` vendor compatibility profile documented in `docs/vendor-regression.md`.
