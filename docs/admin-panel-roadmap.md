...
- Admin maintainability + scale slice delivered: `HsmAdminService` now delegates object browsing to focused helpers, `Keys.razor` / `Pkcs11Lab.razor` moved into partial code-behind files, keys browsing gained server-driven paging with a streaming handle cursor, and summary reads now use batched attribute fetches (`docs/admin-architecture.md`)
