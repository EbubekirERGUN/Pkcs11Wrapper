...
3. `admin-slots.png` — live loaded slot/token visibility surface
...
- The slots image is captured only after the page reports a loaded slot inventory, so the committed README no longer shows the weaker near-empty pre-load state.
