...
node_script="$script_dir/capture-admin-showcase.mjs"
...
artifact_root="${CI_ARTIFACT_ROOT:-$repo_root/artifacts/showcase/admin}"
...
if [[ "$use_existing_env" == "true" || "${PKCS11_USE_EXISTING_ENV:-0}" == "1" ]]; then
  ...
  fixture_root="$(mktemp -d -t pkcs11wrapper-admin-showcase-XXXXXX)"
...
if [[ "$no_restore" != "true" ]]; then
  dotnet restore "$repo_root/src/Pkcs11Wrapper.Admin.Web/Pkcs11Wrapper.Admin.Web.csproj"
  dotnet restore "$repo_root/tests/Pkcs11Wrapper.Admin.E2E/Pkcs11Wrapper.Admin.E2E.csproj"
fi

if [[ "$no_build" != "true" ]]; then
  admin_build_args=(build "$repo_root/src/Pkcs11Wrapper.Admin.Web/Pkcs11Wrapper.Admin.Web.csproj" -c Release)
  e2e_build_args=(build "$repo_root/tests/Pkcs11Wrapper.Admin.E2E/Pkcs11Wrapper.Admin.E2E.csproj" -c Release)
  if [[ "$no_restore" == "true" ]]; then
    admin_build_args+=(--no-restore)
    e2e_build_args+=(--no-restore)
  fi

  dotnet "${admin_build_args[@]}"
  dotnet "${e2e_build_args[@]}"
fi

playwright_package_dir="$repo_root/tests/Pkcs11Wrapper.Admin.E2E/bin/Release/net10.0/.playwright/package"
playwright_cli="$playwright_package_dir/cli.js"
if [[ ! -f "$playwright_cli" ]]; then
  printf 'Playwright CLI not found: %s\n' "$playwright_cli" >&2
  exit 1
fi
node "$playwright_cli" install chromium 2>&1 | tee "$artifact_root/playwright-install.log"

admin_data_root="$(mktemp -d -t pkcs11wrapper-admin-showcase-storage-XXXXXX)"
admin_user="showcase-admin"
admin_password="Showcase!Pass123"
...
profiles = [
    {
        ...
        'Notes': 'Seeded showcase device profile for admin runtime capture',
        'IsEnabled': True,
        'CreatedUtc': now,
        'UpdatedUtc': now,
    }
...
export SHOWCASE_BASE_URL="$admin_base_url"
export SHOWCASE_USERNAME="$admin_user"
export SHOWCASE_PASSWORD="$admin_password"
export SHOWCASE_DEVICE_NAME="$admin_device_name"
export SHOWCASE_MODULE_PATH="$PKCS11_MODULE_PATH"
export SHOWCASE_TOKEN_LABEL="$PKCS11_TOKEN_LABEL"
export SHOWCASE_USER_PIN="$PKCS11_USER_PIN"
export SHOWCASE_FIND_LABEL="$PKCS11_FIND_LABEL"
export SHOWCASE_ARTIFACT_ROOT="$artifact_root"
export SHOWCASE_PLAYWRIGHT_PACKAGE_DIR="$repo_root/tests/Pkcs11Wrapper.Admin.E2E/bin/Release/net10.0/.playwright/package"

(
  cd "$repo_root"
  node eng/capture-admin-showcase.mjs
) 2>&1 | tee "$artifact_root/showcase-capture.log"

find "$artifact_root" -maxdepth 2 -type f | sort > "$artifact_root/artifact-files.txt"
printf 'Admin showcase capture completed successfully. Artifacts: %s\n' "$artifact_root"
